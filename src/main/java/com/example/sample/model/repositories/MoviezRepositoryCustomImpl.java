...
//	private static final String serviceId = "serviceId";
	...
	public List<Moviez> findMovies(MovieFilter filter) throws Exception {
		...
		if (filter.getWatched() != null) {
			criteria = builder.and(criteria, builder.equal(requestRoot.get(watched), filter.getWatched()==true?1:0));
		}
	...
	//	final String status = "status";
		if (filter.getWatched() != null) {
			...
			criteria = builder.and(criteria, builder.equal(serviceRequestRoot.get(watched), filter.getWatched()==true?1:0));
		}
